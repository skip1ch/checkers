...
export default function GamePage({ mode, level, roomCode, myColor, oppName, selectedEmojis = ['shush','wait','cry','lol','shake','clap','think'], onGameEnd, navigate }) {
  ...
  // Fire stats once when game ends
  ...
  // 10-minute draw timer
  useEffect(() => {
    if (!gameResult && timer >= DRAW_SECONDS) {
      setGameResult('DRAW')
    }
  }, [timer, gameResult])
  ...
  // Friend channel — setup with board sync
  useEffect(() => {
    ...
    // Board state sync — other player asks for current state
    ch.on('broadcast', { event: 'state_request' }, () => {
      if (historyRef.current.length > 0) {
        ch.send({
          type: 'broadcast',
          event: 'state_sync',
          payload: { board: boardRef.current, wt: wtRef.current, history: historyRef.current },
        })
      }
    })

    // Board state sync — we receive current state after reconnecting
    ...
    ch.subscribe((status) => {
      if (status === 'SUBSCRIBED') {
        // Request state in case we just reconnected and missed moves
      ...
      {gameResult && (
        <div className="go-overlay">
          <div className="go-card">
            ...
            {isDraw && (
              <div className="go-trophy-delta neutral">Без изменений 🏆</div>
            )}
